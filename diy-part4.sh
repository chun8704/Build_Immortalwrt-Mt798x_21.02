...
sed -i 's/ImmortalWrt/SiYWRT_$(TZ=UTC-8 date "+%Y%m%d")/g' package/base-files/files/bin/config_generate
...
sed -i "25a\echo \"DISTRIB_DESCRIPTION=\'SiY Compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ ImmortalWrt 21.02 \'\" >> /etc/openwrt_release" package/emortal/default-settings/files/99-default-settings
